...
successfulJobs.each{job -> println "Job: ${job.name} took ${job.lastBuild.getDurationString()} to build"}
println "Current Fail job:"
faildJobs.each{job -> println "Job: ${job.name} took ${job.lastBuild.getDurationString()} to build"}
